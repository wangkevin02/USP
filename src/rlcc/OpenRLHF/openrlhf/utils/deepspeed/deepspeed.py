...
def check_environment(rank=0):
    ...
    print(f"\n{'='*80}")
    print(f"[Rank {rank}] Before Save 分布式训练环境信息:")
    print(f"{'='*80}")
    print(f"• 进程信息:")
...
class DeepspeedStrategy(ABC):
    ...
    def save_ckpt(self, model, save_dir, tag=None, max_num=3, max_mem=400, client_state={}, save_latest=True):
        ...
        if self.is_rank_0():
            ...
            while True:
                ...
                if len(subdirs) >= max_num or total_size > MAX_SIZE:
                    ...
                    break
        rank = dist.get_rank()
        print(f"rank {rank} is about to step into barrier")
        check_environment(rank)
        dist.barrier(device_ids=[rank])
        print(f"rank {rank} is out of barrier")
