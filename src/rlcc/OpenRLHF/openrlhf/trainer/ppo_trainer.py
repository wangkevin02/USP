...
def check_environment(rank=0, notion="After Train"):    
    # 在每个进程开始时打印环境信息
    env_info = {
        'global_rank': rank,
    ...
    print(f"\n{'='*80}")
    print(f"[Rank {rank}] {notion} 分布式训练环境信息:")
    print(f"{'='*80}")
    print(f"• 进程信息:")
    print(f"  - 全局进程号 (Global Rank): {env_info['global_rank']}")
    print(f"  - 本地进程号 (Local Rank): {env_info['local_rank']}")
    print(f"  - 总进程数 (World Size): {env_info['world_size']}")
    print(f"  - 可见设备 (Visible Devices): {env_info['cuda_devices']}")
...
class PPOTrainer(ABC):
    ...
    def fit(
        ...
        num_rollouts_per_episodes = (
            num_update_steps_per_episodes # 更新的次数， = num_samples // args.train_batch_size
        ...
        print(f"consumed_samples: {consumed_samples}, global steps: {steps}")
        ...
        for episode in range(start_episode, args.num_episodes):
            ...
            print(f"len of prompts_dataloader: {len(self.prompts_dataloader)}")
            for rand_prompts in self.prompts_dataloader:
                ...
                rank = dist.get_rank()
                check_environment(rank, "Before Train")
                ...
                check_environment(dist.get_rank())
    ...
    def ppo_train(self, global_steps=0):
        ...
        dataloader = DataLoader(
            self.replay_buffer,
            batch_size=self.replay_buffer.sample_batch_size, # micro_train_batch_size
            shuffle=True,
            drop_last=True,
        ...
        for experience in self.replay_buffer:
            print(f"[Rank {rank}] experience from replay buffer: {experience.sequences.shape}")
        for epoch in range(self.max_epochs):
            ...
            for experience in pbar:
                print(f"[Rank {dist.get_rank()}] experience from shared dataloader: {experience.sequences.shape}")
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        rank = dist.get_rank()      
        print(f"[Rank {rank}] actor_sequences: {sequences.size()}")
        print(f"[Rank {rank}] actor_old_action_log_probs: {old_action_log_probs.size()}")
        print(f"[Rank {rank}] actor_action_log_probs: {action_log_probs.size()}")
        print(f"[Rank {rank}] check if actor probs is the same as old probs: {action_log_probs == old_action_log_probs}")
        print(f"[Rank {rank}] actor_advantages: {advantages.size()}")
        ...
        print(f"[Rank {rank}] actor_loss: {actor_loss}")
        # mixtral
        if self.aux_loss:
    ...
    def save_logs_and_checkpoints(self, args, global_step, step_bar, logs_dict={}, client_states={}):
        ...
        if global_step % args.save_steps == 0:
            ...
            print(f"Saving checkpoint at {tag}")
